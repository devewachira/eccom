...
router.post('/add', async (req, res) => {
  try {
    ...
    await mailer.sendEmail(email, 'merchant-application');
...
router.put('/:id/active', auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await mailer.sendEmail(merchantDoc.email, 'merchant-deactivate-account');
    }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await mailer.sendEmail(email, 'merchant-welcome', null, name);
    ...
    await mailer.sendEmail(email, 'merchant-signup', host, {
      resetToken,
      email
    });
